...
async function main() {
  ...
  try {
    ...
    child_process.execSync('pulumi stack select e2e-test --create', { env: { ...process.env, BASELINE_CONFIG_PATH: 'scripts/e2e-config.json',  PULUMI_CONFIG_PASSPHRASE: pulumiPassphrase } });
    ...
    const initialPasswordsStr = child_process.execSync('pulumi stack output --show-secrets initialPasswords --json', { 
      encoding: 'utf-8', 
      env: { 
        ...process.env, 
    ...
    const amiResult = child_process.execSync(`aws ec2 describe-images --owners amazon --filters "Name=name,Values=al2023-ami-2023.*-x86_64" --query "sort_by(Images, &CreationDate)[-1].ImageId" --output text`, { encoding: 'utf-8' }).trim();
    
    // Launch instance
    const runResult = child_process.execSync(`aws ec2 run-instances --image-id ${amiResult} --instance-type t3.nano --query "Instances[0].InstanceId" --output text`, { encoding: 'utf-8' }).trim();
    ...
    const topicsResult = child_process.execSync('aws sns list-topics --query "Topics[*].TopicArn" --output json', { encoding: 'utf-8' });
    ...
    child_process.execSync(`aws sns publish --topic-arn "${killSwitchTopic}" --message "E2E Test Simulation"`);

    console.log("Waiting for Lambda to execute and stop the instance...");
    let stopped = false;
    for (let i = 0; i < 20; i++) {
      const state = child_process.execSync(`aws ec2 describe-instances --instance-ids ${runResult} --query "Reservations[0].Instances[0].State.Name" --output text`, { encoding: 'utf-8' }).trim();
